...
SAMBANOVA_MODEL = "sambanova/gpt-oss-120b"
...
Guidelines:
1. Use Semantic Scholar or Google Scholar to fetch new data. ALWAYS set the limit to 5 results.
...
def create_tool_wrapper(session, tool_info, source_name, on_papers_found=None):
    ...
    async def wrapper(**kwargs):
        ...
        try:
            ...
            return text_content[:12000] + ("..." if len(text_content) > 12000 else "")
        except Exception as e:
    ...
    for param_name, param_info in props.items():
        ...
        annotation = int if json_type == "integer" else (bool if json_type == "boolean" else (list if json_type == "array" else str))
...
class ResearchAgent:
    ...
    async def initialize(self):
        ...
        try:
            ...
            agent = LlmAgent(
                ...
                model=LiteLlm(model=SAMBANOVA_MODEL),
    ...
    async def process_message(self, message: str, session_id: str):
        ...
        input_msg = types.Content(role='user', parts=[types.Part(text=message)])
