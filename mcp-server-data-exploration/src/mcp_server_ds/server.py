...
# Initialize VADER analyzer globally so it is available to all scripts without re-initialization
...
class ScriptRunner:
    ...
    def load_csv(self, csv_path: str, df_name:str = None):
        ...
        try:
            ...
            raise McpError(
                ErrorData(code=INTERNAL_ERROR, message=f"Error loading CSV: {str(e)}")
            ) from e
    ...
    def safe_eval(self, script: str, save_to_memory: Optional[List[str]] = None):
        ...
        # Prepare execution environment by merging libraries and data
        # Using a single dictionary for globals ensures that functions defined in the script
        # can access variables defined in the script (fixing scope issues).
        execution_env = {
            'pd': pd, 
            'np': np,
            'scipy': scipy,
        ...
        if save_to_memory:
            for df_name in save_to_memory:
                ...
                self.data[df_name] = execution_env.get(df_name)
